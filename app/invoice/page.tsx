...
export default function InvoicePage() {
  ...
  useEffect(() => {
    ...
    const el = descriptionRefs.current[lastAddedId];
    if (!el) return;
    setTimeout(() => {
  ...
  const updateLine = (
    id: string,
    patch: Partial<Pick<LineItem, "description" | "quantity" | "unitPrice">>
  ) => {
    setLineItems((prev) =>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Fill out the details and export a clean PDF.
                </p>
              </div>
            ...
            <section className="space-y-4">
              ...
              <div className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-50">
                <div className="grid grid-cols-12 gap-3 border-b border-slate-200 bg-gradient-to-b from-slate-100 to-slate-50 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-slate-600">
                  ...
                  <div className="col-span-8 sm:col-span-3 sm:text-right">
                    Unit price
                  </div>
                  <div className="col-span-8 hidden sm:col-span-2 sm:block sm:text-right">
                    Total
                  </div>
                </div>
                ...
                <div className="space-y-2 p-3">
                  {lineItems.map((item, index) => (
                    <div
                      key={item.id}
                      className="grid grid-cols-12 gap-3 rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200"
                    >
                      ...
                      <div className="col-span-4 sm:col-span-2">
                        <input
                          inputMode="decimal"
                          className={`h-10 w-full rounded-xl border bg-white px-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 sm:text-center ${
                            invalid[`qty-${item.id}`]
                              ? "border-rose-300"
                              : "border-slate-200"
                          }`}
                          value={item.quantity}
                          onChange={(e) =>
                            handleNumberChange(`qty-${item.id}`, e.target.value, (v) =>
                      ...
                      <div className="col-span-8 sm:col-span-3">
                        <div className="relative">
                          ...
                          <input
                            inputMode="decimal"
                            className={`h-10 w-full rounded-xl border bg-white pl-7 pr-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 sm:text-right ${
                              invalid[`unit-${item.id}`]
                                ? "border-rose-300"
                                : "border-slate-200"
                            }`}
                            ...
                            onChange={(e) =>
                              handleNumberChange(
                                `unit-${item.id}`,
                                e.target.value,
                                (v) => updateLine(item.id, { unitPrice: v })
                              )
                            }
                            placeholder="0.00"
                          />
                      ...
                      <div className="col-span-12 sm:col-span-2 sm:text-right">
                        <div className="flex items-center justify-between gap-2 sm:justify-end">
                          ...
                          <button
                            ...
                            className="inline-flex h-9 w-9 items-center justify-center rounded-full text-slate-400 transition hover:bg-rose-50 hover:text-rose-700 active:translate-y-px"
                ...
                <div className="border-t border-slate-200 bg-white px-3 py-3">
                  <button
                    type="button"
                    onClick={addLine}
                    className="inline-flex items-center justify-center rounded-full border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700 shadow-sm transition hover:bg-indigo-100 active:translate-y-px"
                  >
                    + Add Line Item
                  </button>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-3">
                  ...
                  <div className="w-full max-w-[160px]">
                    <div className="relative">
                      ...
                      <input
                        inputMode="decimal"
                        className={`h-10 w-full rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20 ${
  discountMode === "fixed" ? "pl-7 pr-3 text-left" : "pl-3 pr-7 text-right"
}`}
                        value={discountValue}
          ...
          <div className="w-full max-w-lg overflow-hidden rounded-3xl border border-white/10 bg-white shadow-2xl shadow-black/40">
            ...
            <div className="space-y-4 px-6 py-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="text-slate-600">Invoice Number</div>
                <div className="text-right font-semibold text-slate-900">
                  {invoiceNumber || "—"}
                </div>

                <div className="text-slate-600">Sender Name</div>
                <div className="text-right font-semibold text-slate-900">
                  {senderName || "—"}
                </div>

                <div className="text-slate-600">Client Name</div>
                <div className="text-right font-semibold text-slate-900">
                  {clientName || "—"}
                </div>
                ...
                <div className="text-slate-600">Line Items</div>
                <div className="text-right font-semibold text-slate-900">
                  {
                    lineItems.filter(
