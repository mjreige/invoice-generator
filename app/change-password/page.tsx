...
export default function ChangePasswordPage() {
  ...
  const validateFields = () => {
    ...
    else if (!passwordRules.every(rule => rule.test(newPassword))) errors.newPassword = "Please meet all password requirements";
    ...
    if (currentPassword === newPassword) errors.newPassword = "New password must be different from current password";
        ...
        <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/95 shadow-2xl shadow-black/40 backdrop-blur">
          ...
          <form onSubmit={handleSubmit} className="space-y-5 px-6 py-6 sm:px-8 sm:py-8">
            <div className="space-y-1.5">
              <label htmlFor="currentPassword" className="block text-xs font-semibold uppercase tracking-wider text-slate-700">Current password</label>
              <div className="relative">
                <input
                  ...
                  required
                  className={`h-11 w-full rounded-2xl border bg-slate-50 px-4 pr-20 text-sm text-slate-900 shadow-sm outline-none transition focus:bg-white focus:ring-2 ${fieldErrors.currentPassword ? "border-rose-300 focus:border-rose-500 focus:ring-rose-500/20" : "border-slate-200 focus:border-blue-500 focus:ring-blue-500/20"}`}
                  value={currentPassword}
                  onChange={(e) => { setCurrentPassword(e.target.value); if (fieldErrors.currentPassword) setFieldErrors(prev => ({ ...prev, currentPassword: "" })); }}
                />
                <button type="button" onClick={() => setShowCurrentPassword(!showCurrentPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 hover:text-slate-700">
                  {showCurrentPassword ? "Hide" : "Show"}
                </button>
              </div>
              {fieldErrors.currentPassword && <p className="text-xs text-rose-600">{fieldErrors.currentPassword}</p>}
            </div>

            <div className="space-y-1.5">
              <label htmlFor="newPassword" className="block text-xs font-semibold uppercase tracking-wider text-slate-700">New password</label>
              <div className="relative">
                <input
                  id="newPassword"
                ...
                />
                <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 hover:text-slate-700">
            ...
            <div className="space-y-1.5">
              <label htmlFor="confirmNewPassword" className="block text-xs font-semibold uppercase tracking-wider text-slate-700">Confirm new password</label>
              <div className="relative">
                <input
                  ...
                  onChange={(e) => { setConfirmNewPassword(e.target.value); if (fieldErrors.confirmNewPassword) setFieldErrors(prev => ({ ...prev, confirmNewPassword: "" })); }}
                />
                <button type="button" onClick={() => setShowConfirmNewPassword(!showConfirmNewPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 hover:text-slate-700">
